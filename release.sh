...
for GOOS in linux windows darwin; do
    for GOARCH in amd64; do
        ...
        GOOS=$GOOS GOARCH=$GOOARCH go build -ldflags "-X main.version=$APP_VERSION" -o "build/kepubify-$GOOS-$GOARCH$(echo $GOOS|sed 's/windows/.exe/g'|sed 's/linux//g'|sed 's/darwin//g')"
    done
done
